...
Adapted from coreyhaines31/marketingskills (MIT), `skills/social/references/carousel-frameworks.md`.
...
- Create the post with all slides as `media_urls` in order (slide 1 first — it's
  the cover). Platform caps: check marky-api's platform rules for max carousel
  slides per platform before authoring.
- Tag the post `metadata` with `format: carousel` and `hook: <framework letter>`
  so review-performance can score frameworks against each other for THIS
  audience.
- The caption is a second hook, not a repeat of slide 1 — write it per the
  humanize skill's standards.
